...
(function ($) {
    ...
    BootstrapTreeTable.prototype = {
        ...
        dataHandler: function (element) {
            ...
            // element.data('leaf', childrenlist.length === 0 ? 1 : 0); // 1: leaf,0: not leaf // 朱才富于2020-09-07 11:14 注释，改为由外部设置
        ...
        setbranch: function (element) {
            ...
            var isLeaf = element.data('leaf') === 0
            $td.wrapInner("<span class='nodename " + (isLeaf ? 'leaf' : 'nonleaf') + "'></span>");
            if (isLeaf) {
                var $a = $('<a href="#"></a>').addClass('nodepoint').css('padding-right', '5px').append(this.options.collapsedIcon)
                $td.prepend(this.options.nonLeafIcon).prepend($a);
            } else {
                var $span;
            ...
            this.options.showTools && element.data('tool') != 'false' && $td.append(this.options.toolIcon);
        },
        /**
         * 页面初始化层级展示
         * @param element
        ...
        clickListener: function () {
            ...
            this.$element.on('click.bs.treetable', '.nodepoint', function (e) {
                e.preventDefault();
        ...
        collapseHandler: function (element) {
            this.collapseById(element.data('id'));
        },

        collapseById: function (id) {
            var that = this,
                childrenlist = this.nodes.filter('tr[data-pid=' + id + ']');
            childrenlist.hide();
            childrenlist.each(function () {
                that.collapseById($(this).data('id'));
            })
        },
        ...
        expendHandler: function (element) {
            ...
            var id = element.data('id');
            var childrenlist = this.nodes.filter('tr[data-pid=' + id + ']');
            var filter = this.options.filter, isFilter = typeof filter === "function";
            childrenlist.each(function () {
                var $this = $(this);
                // 过滤显示
                ((isFilter && filter($this, $this.data('leaf') == 0)) || !isFilter) && $this.show();
            });
        ...
        filterAll: function(){
            var that = this,
                ...
                isFilter = typeof filter === "function";
            this.$element.find('tbody > tr').each(function(){
                var $this = $(this), isFolder = $this.data('leaf') == 0;
                if(isFilter){
                    var isShow = filter($this, isFolder);
                    !isShow && isFolder && that.collapseHandler($this);
        ...
        /**
         * 根据节点删除节点（朱才富修改：2020-09-07）
         */
        removeByNode: function (node) {
            var nodepid = node.data('pid');
            node.remove();
            // 判断是否还存在子节点，不存在则合并
            var children = this.nodes.filter("tr[data-pid='" + nodepid + "']");
            children.length === 0 && this.collapseById(nodeid);
        },
        // removeByNode: function (node) {
        //     var that = this;
        ...
        //         // 重置parent节点
        //         that.setNode2Leaf(nodepid); // 不需要重置父节点，朱才富注释
        ...
        /**
         * 移除非叶子节点 （朱才富添加：2020-09-07）
         * @param {String} id 节点 id
         */
        removeNonLeafById: function (id) {
            var that = this,
                childrenlist = this.nodes.filter('tr[data-pid=' + id + ']');
            childrenlist.hide();
            childrenlist.each(function () {
                let eId = $(this).data('id');
                that.removeNonLeafById(eId);
                that.removeById(eId);
            })
        },
        ...
        /**
         * 添加节点（朱才富添加）
         * @param {String} preId 添加到哪一个节点之后
         * @param {Int} level 层级
         * @param {JQuery} node 节点
         */
        appendNode: function (preId, level, node) {
            var that = this,
                parent = that.nodes.filter('tr[data-id=' + node.data('pid') + ']'),
                preNode = that.nodes.filter('tr[data-id=' + preId + ']'),
                column = that.options.column !== '' ? that.options.column : 0,
                levelSpacing = that.options.levelSpacing !== '' ? that.options.levelSpacing : 20,
                $td = node.children().eq(column);
            $td.css('padding-left', level * levelSpacing + 'px');
            node.data('level', level);
            that.setbranch(node);
            !parent.hasClass('expended') && node.hide(); // 如果父节点没有展开，则不显示
            preNode.after(node);
            that.nodes.push(node.get(0));
        },
        /**
         * 批量添加子节点（朱才富添加）
         ...
         */
        appendChildrenNode: function (pid, nodes) {
            var that = this,
                parent = that.nodes.filter('tr[data-id=' + pid + ']'),
                level = Number(parent.data('level')) + 1;
            if(Array.isArray(nodes)){
                for(let i = nodes.length-1; i >= 0; i--){
                    this.appendNode(pid, level, nodes[i]);
                }
            }
        }
    };
