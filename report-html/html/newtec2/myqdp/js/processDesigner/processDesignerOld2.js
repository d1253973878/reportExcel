...
//	console.log(Raphael);
	...
	/*创建流程设计器*/
	processDesigner.create = function(processContainer,closeEle){
...
//		alert("offsetWidth: " +processContainer.offsetWidth+"offsetHeight: " + processContainer.offsetHeight );
		...
		var cancelBtn = Newtec.Button.create({
		    title: '返回',
		    className: 'btn btn-default btn-lg',
		    click: function () {
		    	$("#processDiv").remove();
        		$(".compexSearchPage ").show("slow");//TODO
		    	
		    }
		});
	    var cancalDiv = $("<div id='cancelDiv'></div>").append(cancelBtn.newtecJQ);
	    $('#processDiv').append(cancalDiv);
...
//		setTimeout(function(){upbtn(); },100);//下方上下按钮
	...
	/*创建流程设计器并读取流程*/
	processDesigner.loadProcess = function(processContainer,processId){
		processDesigner.create(processContainer);
		load(processId);
	}
	/*关闭流程设计器*/
	var closeProcessDesigner = function(){
		var a = document.getElementById("processDiv")
...
//				32);
		var rightbtn = paper.image(rightbtnBSrc,
				getPaperWidth() - 56+17, getPaperHeight() / 2, 48,
				32);
...
//				window.setTimeout(function(){background.click(click);},100);//延迟回复click事件
	...
	function drawNodeTool(){
		var nodeIcon = paper.rect(5,40,30,30,5); //创建长方形 x,y,width,height,radius
...
//			this.templine = connect(node,{flag:1,x:x-getPaperLeft(),y:y-getPaperTop()},lineColor,"curve");
	...
	ProcessNode.prototype.dragMove = function(dx,dy,x,y){			
			if(defaultOn){
				var curx = this.type=="rect"?this.attr("x"):this.attr("cx");
				var cury = this.type=="rect"?this.attr("y"):this.attr("cy");
				this.type=="rect"?this.attr({x: this.ox + dx, y: this.oy + dy}):this.attr({cx: this.ox + dx, cy: this.oy + dy});
//				/*调整快捷工具栏*/
...
//				this.text.show();//拖动结束显示文字
	...
	ProcessNode.prototype.createShape = function (){
		...
		node.quickToolBar = createQuickToolBar(node.attr("x"),node.attr("y"),tools,node);
		/*定义文字对象*/	
		...
		var renameEve = function(){
			...
			input.style.left = (node.type=="rect"?node.attr("x"):node.attr("cx")-node.attr("r")) + 1 +"px";	
			input.style.top = (node.type=="rect"?node.attr("y"):node.attr("cy")-node.attr("r")/2)+1 +"px";
			input.style.width = (node.type=="rect"?node.attr("width"):node.attr("r")*2-2)-2+"px";
			input.style.height = (node.type=="rect"?node.attr("height"):20)-2+"px";
	...
	var ProcessLine  = function(obj1, obj2, path,type,color,bg){
		...
		{ 
			...
			realLine.dechoose = function(){
				this.ischoose=false;
				this.attr({stroke:"black","stroke-dasharray":""});
				choosingObjs.splice(choosingObjs.indexOf(this),1);
			}	
	...
	function resetTextPos(node){
		if(node.type=="rect"){//定义节点自己的快速工具栏  
			node.text.attr({x:node.attr("x")+node.attr("width")/2,y:node.attr("y")+node.attr("height")/2});
			}else{
	...
	function createTextDragEve(text,node){
		var dragmove = function(dx,dy,x,y){			
			if(defaultOn){
				var curx = node.type=="rect"?node.attr("x"):node.attr("cx");
				var cury = node.type=="rect"?node.attr("y"):node.attr("cy");
...
//				node.oy = nodey+e.offsetY;
	...
	function initForm(){ //业务
		...
		processForm = Newtec.Form.create({
			...
			fields:[
			    //{name:'id',title:'id'},
			   	{name: 'key1', title: '流程key',required:"不能为空"},
			   	{name: 'version', title: '版本',required:"不能为空"},
			   	{name: 'name', title: '流程名称',required:"不能为空"},
				{name: 'status', title: '状态',data: {'0':'禁用','1':'启动'}, type: 'select',showDownIcon:true,search:false,required:"不能为空"},
				{name: 'createTime', title: '创建时间',type:'datetime',required:"不能为空",valiType:"datetime"},
				{name: 'updateTime', title: '最后更新时间',type:'datetime',valiType:"datetime"},
				{name: 'createPersonName', title: '创建人名称'},
				{name: 'createDepartmentId', title: '部门名称',type: 'select',ds:{name:'department',sortBy:{id:"desc",name:"asc"},endRow:1000},maxHeight:150},
				{name: 'description', title: '描述'},
				{name: 'imports', title: '流程需要的导入包'},
				{name: 'bussClass', title: '业务实体类路径'},
				{name: 'areaDesc', title: '区域备注'},
				{name: 'startProcessBefore', title: '开始流程前'},
				{name: 'startProcessAfter', title: '开始流程后'},
				{name: 'finshProcessBefore', title: '流程完成前'},
				{name: 'finshProcessAfter', title: '流程完成后'},
			],
		});
//		 nodeForm = Newtec.Form.create({appendTo:'#nodeForm',ds:Newtec.DS.get('bPMNode'),autoFetch:true,titleColumn:"3",columnNum:"2"});
//		 lineForm=	Newtec.Form.create({appendTo:'#lineForm',ds:Newtec.DS.get('line'),autoFetch:true,titleColumn:"3",columnNum:"2"});
...
//            	}
            	if(res.status==0){
            		Newtec.Window.createHint({html: '<span>操作成功<span>'});
            		$("#processDiv").remove();
            		$(".compexSearchPage ").show("slow");//TODO
            		maxTable.fetchData();
            	}
