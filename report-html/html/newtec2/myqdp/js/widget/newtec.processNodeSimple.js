;(function(){
	...
	var fontFace = "微软雅黑";
	Newtec.ProcessNodeSimple = function(params){
		this.defaults ={//默认参数
			nodeType : ''	
		};
		this.currNode ={};
	...
	Newtec.ProcessNodeSimple.prototype = new Newtec.Base('processNode');
	Newtec.ProcessNodeSimple.prototype.createNewtecJQ = function(params){
//		initForm();
//		formFieldEve();
...
//		var node = "<p>进入.....</p>";
	...
	Newtec.ProcessNodeSimple.create = function(params){
		return new Newtec.ProcessNodeSimple(params).init();
	};
	Newtec.ProcessNodeSimple.getCurFormEle = function(){
		return curShowFormEle;
	};
	Newtec.ProcessNodeSimple.getCurShowForm = function(){
		return curShowForm;
	};
	Newtec.ProcessNodeSimple.setCurFormEle = function(formEle){
		curShowFormEle = formEle;
	};
	Newtec.ProcessNodeSimple.setCurShowForm = function(from){
		curShowForm = from;
	};
	Newtec.ProcessNodeSimple.getUndoStack = function(){
		return undoStack;
	};
	Newtec.ProcessNodeSimple.getRedoStack = function(){
		return redoStack;
	};
	Newtec.ProcessNodeSimple.setUndoStack = function(stack){
		undoStack = stack;
	};
	Newtec.ProcessNodeSimple.setRedoStack = function(stack){
		redoStack = stack;
	};
	Newtec.ProcessNodeSimple.setNodeFormEle = function(formEle){
		nodeFormEle = formEle;
	};
	Newtec.ProcessNodeSimple.setLineFormEle = function(formEle){
		lineFormEle = formEle;
	};
	Newtec.ProcessNodeSimple.getCurNode = function(){
		return curNode;
	};
	Newtec.ProcessNodeSimple.getCurLine = function(){
		return curLine;
	};
	Newtec.ProcessNodeSimple.setLineBtnOn = function(type){
		type == "curve" ? curlineOn = true
				: type == "poly" ? polylineOn = true
						: straightlineOn = true;
	};
	Newtec.ProcessNodeSimple.setLineBtnOff = function(type){
		type == "curve" ? curlineOn = false
				: type == "poly" ? polylineOn = false
						: straightlineOn = false;
	};
	Newtec.ProcessNodeSimple.setDefaultOn = function(isDefaultOn){
	...
	ProcessNode.prototype.mouseoverEve = function() {// 鼠标移进事件
		if(this.procNode.nodeProp.name!="完成点"){
			var div = document.getElementById("nodeDetail2");
			//console.log(this.attr("cx"));
			div.style.left = (this.attr("cx")+5)+"px";
			div.style.top = this.attr("cy")+"px";
			div.style.position = "absolute";
			div.style.display = "block";
			div.innerHTML ="";
			var info = this.procNode.approvalInfo;
...
//		this.height = height;
		...
		this.getRealShape = function(){
			return paper.circle(this.left+this.r/2,this.top+this.r/2,r).attr({"fill":"gray","stroke-width":"0.5",stroke:"black"});
		};
	...
	Newtec.Module("ProcessNodeSimple")
})();