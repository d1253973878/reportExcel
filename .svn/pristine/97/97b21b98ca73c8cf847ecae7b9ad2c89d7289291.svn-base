;(function(){
	...
	var showTable = true;
	var showDigram = true;
	Newtec.ApprovalProcess=function(params){
		this.defaults = {//默认参数
				bussId:''
			};
	...
	Newtec.ApprovalProcess.prototype = new Newtec.Base('approvalHistory');
	
	Newtec.ApprovalProcess.prototype.createNewtecJQ=function(f){

		var processContainer = document.getElementById(f.appendTo);
		console.log(f.appendTo);
		...
		paperHeight = processContainer.offsetHeight-7-250;
		...
		window.onresize = function(){ // 注册window大小监听事件  窗口大小改变时paper大小改变
			...
			var curHeight = processContainer.offsetHeight-7-250; 
...
//		nodeDetail.top = '250';
	...
	Newtec.ApprovalProcess.create = function(params){
		return new Newtec.ApprovalProcess(params).init();
	};
		...
		Newtec.DS.get("processTest").fetchData({
			...
			callback:function(res){
				...
				if(showDigram){
					...
					load(key,nodeArr,arrowArr);//读取流程信息并画流程图
				}
			}
		});
	};
	...
	var drawAndSetData = function(process,pnodes,plines,premarks,nodeArr,arrowArr){
		...
		for(var i=0;i<pnodes.length;i++){
			...
			if(!Newtec.Utils.isNull(nodeArr[pnode.nodeFlag])){//审过的节点都有相应的颜色填充，没审过的都为默认颜色
				...
				if(node.getShape().type=="circle"){
					paper.text(node.getShape().attr("cx"),node.getShape().attr("cy")+50, node.approvalInfo.endTime.replace(' ','\n')).attr({
						cursor : "default",
						"font-family" : "微软雅黑",
						"font-size" : "10",
						"fill":"black"
					...
					if(endTime){
						paper.text(node.getShape().attr("x") + node.getShape().attr("width") / 2,
								node.getShape().attr("y") + node.getShape().attr("height") / 2+35,
								endTime.replace(' ','\n')).attr({
							cursor : "default",
							"font-family" : "微软雅黑",
							"font-size" : "10",
	...
	var approvalInfo = function(){//流程审批过程数据
		this.nodeName = '';
		this.deptName = '';
		this.personName = '';
		this.state = '';
	...
	var showHistoryTable = function(data){
		var dataFields = [
