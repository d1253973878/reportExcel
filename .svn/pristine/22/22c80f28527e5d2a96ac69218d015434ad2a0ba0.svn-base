...
//				}
	...
	Newtec.Component=function(js,css,callback){
		if(Newtec.Utils.isFunction(js)){
				callback=js;
				css=null;js=null;
		}
		...
		if(Newtec.Utils.isNull(js)||(Newtec.Utils.isArray(js)&&js.length==0)){
			isCallBack()
		}else{
			Newtec.Utils.addJS(js,function(){
				isCallBack();
			});
		}
	}
	...
	//检测页面ready调用
// 	$(function(){
// 		console.info("============read=====addCount="+addCount)
// 		Newtec.pageStatus=1;
// 		if(addCount)return;
// 		function deary(){
// 			if(addCount)return;
// 			console.info(addCount+"============loadedJs======"+loadedJs)
// 			if(loadedJs<4){
// 				setTimeout(function(){
// 					deary();
// 				},5)
